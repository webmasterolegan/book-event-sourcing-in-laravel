...
Мы объединим знания двух предыдущих глав и используем части агрегатов для построения конечного автомата. Хотя шаблон "состояние" полезен во многих сценариях, вам может не нравиться, как он распределяет логику, связанную с состоянием, по нескольким классам. Теперь, когда мы знаем, как работают части агрегата, мы можем создать один, предназначенный для управления состоянием агрегата.

Мы начнем с создания CartStateMachine; это класс части агрегата:
...
Внутри мы отслеживаем два соответствующих состояния, которые мы определили ранее: CartStatus и CartLockStatus; на этот раз это снова простые перечисления, так как вся функциональность будет обрабатываться нашим конечным автоматом:
...
Далее, нам нужен механизм для продвижения состояния вперед. Когда мы использовали шаблон "состояние", у нас был метод `nextState()` в каждом классе состояния. Однако, используя части агрегата, мы можем прослушивать события агрегата для изменения состояния:
...
Мы не сделали ничего нового по сравнению с использованием шаблона "состояние", но использование части агрегата в качестве конечного автомата имеет некоторые преимущества:
